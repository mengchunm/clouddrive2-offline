...
- 任务列表中的“网页播放”会在当前页面右下角打开 ArtPlayer 悬浮窗口，不再用遮罩覆盖整页；视频顶部标题和关闭按钮会随控制栏自动显示或隐藏，拖动标题可移动窗口，四角及上下左右边缘均会按当前视频的真实宽高比等比例缩放。位置和大小会自动保存，播放器内部仍可切换为全屏；窗口较小时会自动精简次要控制按钮，避免控制栏被裁切。播放器存在且焦点不在输入框时，可按空格切换播放和暂停。
...
- 视频元素直接使用 CloudDrive2 原始 Range 地址。不要把扩展包内的静态占位资源当作动态媒体代理；Manifest V3 后台无法可靠接管这类资源请求。
