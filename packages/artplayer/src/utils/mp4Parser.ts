...
interface Mp4boxReadyInfo {
	tracks: Mp4boxTrackInfo[];
}

interface Mp4boxTrakBox {
	...
	mdia: {
		...
		minf: {
			stbl: {
				...
				stts?: {
					sample_counts: Array<
						number | { sample_count: number; sample_delta: number }
					>;
...
interface Mp4boxFileLike {
	moov?: { traks: Mp4boxTrakBox[] };
	onError?: (e: unknown) => void;
	onReady?: (info: Mp4boxReadyInfo) => void | Promise<void>;
	appendBuffer(data: Mp4boxArrayBuffer): number | null | undefined;
	flush(): void;
}
...
export async function extractMp4Subtitle(
	...
	return new Promise((resolve, reject) => {
		const mp4boxfile = MP4Box.createFile() as unknown as Mp4boxFileLike;
		let isDone = false;
		let fileOffset = 0;
		...
		mp4boxfile.onReady = async (info: Mp4boxReadyInfo) => {
			if (isDone) return;
			isDone = true;
			...
			for (const track of info.tracks) {
				if (
					...
					const name = track.name || langMap[lang] || lang;
					targetTracks.push({
						id: track.id,
						codec: track.codec,
						language: lang,
			...
			for (const t of targetTracks) {
				try {
					// get the trak box
					const trak = mp4boxfile.moov?.traks.find(
						(tr) => tr.tkhd.track_id === t.id,
					);
					if (!trak) continue;

					const stbl = trak.mdia.minf.stbl;
					const stszBox = stbl.stsz;
					const stsz = stszBox?.sample_sizes;
					const offsets = stbl.stco
						? stbl.stco.chunk_offsets
						: stbl.co64
							? stbl.co64.chunk_offsets
							: null;
					const stscBox = stbl.stsc;
					const stsc =
						stscBox?.samples_table ??
						stscBox?.first_chunk?.map((firstChunk, index) => ({
							first_chunk: firstChunk,
							samples_per_chunk: stscBox.samples_per_chunk?.[index] ?? 0,
						}));
					const sttsBox = stbl.stts;
					const stts = sttsBox?.sample_counts.map((entry, index) =>
						typeof entry === "number"
							? {
									sample_count: entry,
									sample_delta: sttsBox.sample_deltas?.[index] ?? 0,
								}
							: entry,
					);

					if (!stsz || !offsets || !stsc || !stts) {
						console.warn("[mp4Parser] Subtitle sample table is incomplete", {
							trackId: t.id,
							stblKeys: Object.keys(stbl),
							hasSampleSizes: Boolean(stsz),
							hasOffsets: Boolean(offsets),
							hasSampleToChunk: Boolean(stsc),
							hasTimings: Boolean(stts),
						});
						continue;
					}
					...
					for (let i = 0; i < stts.length; i++) {
						const { sample_count: count, sample_delta: delta } = stts[i];
						for (let j = 0; j < count; j++) {
							durations.push(delta);
						}
					}

					const defaultSampleSize = stszBox?.sample_size ?? 0;
					...
					let nextFirstChunk =
						stscIndex + 1 < stsc.length
							? stsc[stscIndex + 1].first_chunk
							: Infinity;
					...
					for (const s of samples) {
						...
						const range = fetchRanges.find(
							(r) => s.offset >= r.start && s.offset + s.size - 1 <= r.end,
						);
		...
		async function startFetch() {
			try {
				const res = await customFetch(videoUrl, {
					headers: {
						Range: `bytes=${fileOffset}-${fileOffset + CHUNK_SIZE - 1}`,
					},
				});
				if (!res.ok && res.status !== 206) {
				...
				const buffer = ab as Mp4boxArrayBuffer;
				buffer.fileStart = fileOffset;
				fileOffset += buffer.byteLength;
				const nextPosition = mp4boxfile.appendBuffer(buffer);
