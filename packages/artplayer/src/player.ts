...
function createDanmakuPanel(playerContainer: HTMLDivElement) {
  ...
  `;
    ...
    const searchBtn = panel.querySelector<HTMLButtonElement>(".cd2-dm-search button")!;
    const body = panel.querySelector<HTMLDivElement>(".cd2-dm-body")!;
    const closeBtn = panel.querySelector<HTMLButtonElement>(".cd2-dm-close-panel")!;
...
function renderMatches(
    ...
    for (const m of matches) {
        (groups.get(m.animeTitle) || (groups.set(m.animeTitle, []), groups.get(m.animeTitle)!)).push(m);
    }
    for (const [title, items] of groups) {
        ...
        for (const item of items) {
            ...
            el.onclick = () => onSelect(item.episodeId, `${item.animeTitle} - ${item.episodeTitle}`);
            g.appendChild(el);
        }
        body.appendChild(g);
    }
...
function renderAnimes(
    ...
    body.innerHTML = "";
    if (animes.length === 0) {
        body.innerHTML = '<div class="cd2-dm-status">无搜索结果，换个关键词试试</div>';
        return;
    }
    for (const anime of animes) {
        ...
        for (const ep of anime.episodes) {
            ...
            el.onclick = () => onSelect(ep.episodeId, `${anime.animeTitle} - ${ep.episodeTitle}`);
            g.appendChild(el);
        }
        body.appendChild(g);
    }
...
export async function openPlayer(url: string, fileName: string, title?: string) {
    ...
    const onSelectEpisode = async (episodeId: number, label: string) => {
        ...
        try {
            ...
            updateControlText();
            if (currentPlayer) currentPlayer.notice.show = `已加载 ${danmaku.length} 条弹幕`;
        } catch (err) {
            danmakuStatusText = "加载失败";
            updateControlText();
            if (currentPlayer) currentPlayer.notice.show = `弹幕加载失败: ${(err as Error).message}`;
        }
    };
...
async function autoMatch(
    ...
    if (!hasCredentials()) {
        setStatus("需配置API密钥");
        panelEls.body.innerHTML = '<div class="cd2-dm-status">请先通过油猴菜单「⚙ 弹弹Play API 配置」设置 AppId 和 AppSecret<br><br>申请方式：发邮件到 kaedei@dandanplay.net<br>主题：弹弹play开放平台申请</div>';
        if (currentPlayer) currentPlayer.notice.show = "弹幕功能需配置弹弹Play API密钥，请在油猴菜单中设置";
        return;
    }
    ...
    try {
        ...
        if (matchResult.isMatched && matchResult.matches.length > 0) {
            ...
            renderMatches(panelEls.body, matchResult.matches, onSelect, match.episodeId);
            await onSelect(match.episodeId, `${match.animeTitle} - ${match.episodeTitle}`);
            return;
        }
        ...
        if (keyword) {
            ...
            if (searchResult.animes.length > 0) {
                ...
                if (best) {
                    await onSelect(best.episodeId, `${best.animeTitle} - ${best.episodeTitle}`);
                    return;
                }

                // 有结果但无法确定集数，提示用户选择
                setStatus(`找到 ${searchResult.animes.length} 部番剧，点击选集`);
                if (currentPlayer) currentPlayer.notice.show = "已搜索到番剧，请点击弹幕按钮选择集数";
                return;
            }
            ...
            if (shortKeyword !== keyword && shortKeyword.length >= 2) {
                console.log("[cd2-artplayer] 策略3: 缩短关键词搜索, keyword=", shortKeyword);
        ...
        panelEls.body.innerHTML = '<div class="cd2-dm-status">自动匹配失败，请手动输入番剧名搜索</div>';
        if (currentPlayer) currentPlayer.notice.show = "未自动匹配到弹幕，可点击弹幕按钮手动搜索";
    } catch (err) {
    ...
    for (const anime of animes) {
        for (const ep of anime.episodes) {
            ...
            if (nums) {
                for (const n of nums) {
                    if (parseInt(n) === epNum) {
                        return { episodeId: ep.episodeId, animeTitle: anime.animeTitle, episodeTitle: epTitle };
                    }
                }
            }
        }
